function varargout=sldemo_tankgui_puc(action)
%sldemo_tankgui_puc GUI for sldemo_tank model
%
%   sldemo_tankgui_puc manages the animation UI for the Simulink tank example.
%   
%   See also sldemo_tank.
...
%
% locate the primary tank gui window
%
...
case 'update'
  ...
  elseif hilim<lolim
    ...
    LocalDrawTank(hilim, lolim, inflow, outflow, area);
  end %end idiot chex
   
case 'stop'
  %halt sim if it is running
  LocalLoadModel(mdl);
  if strcmp(get_param(mdl,'SimulationStatus'),'running')
  ...
  set(h.push.stop, ...
      'String','START SIM'      , ...
      'Callback','sldemo_tankgui_puc start');
...
case 'start'
  ...
  set(h.push.stop, ...
      'String'  ,'STOP SIM'      , ...
      'Callback','sldemo_tankgui_puc stop');
...
%==============================================================
%
function [h,figs]=LocalStartFromScratch

%Setup functions
[hilim, lolim, inflow, outflow, area]=LocalInitialize;
figs = LocalInitFigures;
h=LocalInitButtons(hilim, lolim, inflow, outflow, area);
g=LocalDrawTank(hilim, lolim, inflow, outflow, area); %#ok<NASGU>

%
%==============================================================
% LocalInitialize
% Initializes the tank simulation parameters.
%==============================================================
%
...
%==============================================================
% LocalInitButtons
% Creates the buttons in the main tank GUI figure.
%==============================================================
%
function h=LocalInitButtons(hilim, lolim, inflow, outflow, area) %#ok<INUSD>
...
h.push.close=uicontrol(ui.push, ...
    ...
    'Position', [pos.push.x pos.push.y+(2-1)*height.push ...
                 width.push height.push] , ...
    'Callback'  ,'sldemo_tankgui_puc close');
...
ui.edit.Callback = 'sldemo_tankgui_puc update';
...
%==============================================================
%
function g=LocalDrawTank(hilim, lolim, inflow, outflow, area)

%set up the background graphics/lines for the tank
...
%==============================================================
% LocalInitFigures
% Creates the figures for the tank GUI example.
%==============================================================
%
...
% tankgui figure windows.
%==============================================================
%
function tagStrings = LocalGetFigureTagStrings

tagStrings = { 'TankGUIFigure', 'TankPostRunSummaryFigure' };

%
%==============================================================
% LocalGetFigure
% Return the handle of the index tank GUI figure
%==============================================================
%
...
%==============================================================
% LocalMakeFigure
% Creates the specified tank GUI figure
%==============================================================
%
function handle=LocalMakeFigure(num)

tagStrings       = LocalGetFigureTagStrings;
closeRequestFcns = {'sldemo_tankgui_puc done',   'sldemo_tankgui_puc close'};
figNames         = {'Tank Model GUI', 'Post-Run Summary'};
...
% CloseFcn) or when the 'close' box of the main tank GUI
% figure is clicked.
%==============================================================
%
function LocalDone(mdl)

%
% delete the post-run summary figure
%
f = LocalGetFigure(2);
if ishghandle(f,'figure')
  delete(f)
end

%
% delete the main tank GUI figure
%
f = LocalGetFigure(1);
if ishghandle(f,'figure'),
  h = get(f,'UserData');
  mystring = get(h.push.stop,'String');
  if strncmp(mystring,'STOP',4)
      if bdIsLoaded(mdl)
          %set_param(mdl,'SimulationCommand','stop')
